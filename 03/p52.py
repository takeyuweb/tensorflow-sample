...
# 学習データの管理
# 画像拡張を使って元々の画像からずらしたものを用意し
# 同じ構図ばかりにならないようにする
# これにより過剰適合対策、見たことない構図での適合率向上を見込める
train_datagen = ImageDataGenerator(
    rescale=1/255,
    rotation_range=40,  # 右または左に最大40度までのランダム回転
    width_shift_range=0.2,  # 最大20%の横移動
    height_shift_range=0.2,  # 最大20%の縦移動
    shear_range=0.2,    # 最大20%の剪断変形
    zoom_range=0.2,     # 最大20の拡大
    horizontal_flip=True,   # ランダムに水平または垂直回転
    fill_mode='nearest'  # 移動または剪断後に欠落したピクセルを最近傍の値で充填
)
